...
static void wifi_init()
{
    ...
    static void wifi_esp_now_init(esp_now_peer_info_t peer)
    {
        ...
        esp_now_rate_config_t rate_config = {.phymode = CONFIG_ESP_NOW_PHYMODE,
                                             .rate =
                                                 CONFIG_ESP_NOW_RATE, //  WIFI_PHY_RATE_MCS0_LGI,
                                             .ersu = false,
                                             .dcm = false};
        esp_now_rate_config_t rate_config = {.phymode = CONFIG_ESP_NOW_PHYMODE,
                                             .rate =
        ...
        if (!info || !info->buf)
        {
            ...
            if (memcmp(info->mac, CONFIG_CSI_SEND_MAC, 6))
            {
               ...
               rx_ctrl->rx_state);
                        ...
                        {
                            csi = ((int16_t)(((((uint16_t)info->buf[i + 1]) << 8) | info->buf[i])
                                             << 4) >>
                                   4);
                    ...
                    void temp_callback(void *ctx, wifi_csi_info_t *info)
                    {
                        ...
                        void temp_callback(void *ctx, wifi_csi_info_t *info)
                        {
                            ...
                            esp_csi_gain_ctrl_get_gain_compensation(&compensate_gain, agc_gain,
                                                                    fft_gain); // 各种增益补偿计算
                            // ESP_LOGI(TAG, "compensate_gain %f, agc_gain %d, fft_gain %d",
...
#else
                       wifi_nonow_init();
                       ets_printf("unpupported mode\n");
                       esp_wifi_connect();
                       wifi_csi_init();
                       while (1)
                       {
                           printf("csi recv running\n");
