...
static void wifi_csi_rx_cb(void *ctx, wifi_csi_info_t *info) {
  ...
  if (s_count < 100) {
    ...
    ESP_LOGD(TAG, "fft_force %d, agc_force %d", fft_gain_baseline,
             agc_gain_baseline);
#endif
  }
  ...
  if (!s_count) {
    ...
    ets_printf("type,seq,mac,rssi,rate,noise_floor,fft_gain,agc_gain,channel,"
  ...
  if (!s_count) {
    ESP_LOGI(TAG, "================ CSI RECV ================");
    ets_printf("type,id,mac,rssi,rate,sig_mode,mcs,bandwidth,smoothing,not_"
               "sounding,aggregation,stbc,fec_coding,sgi,noise_floor,ampdu_cnt,"
